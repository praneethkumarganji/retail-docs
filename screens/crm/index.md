 # CRM — Customers, Segments & Campaigns (Overview)

This folder contains the CRM screen specification and detailed feature documentation.

ASCII Layout (detailed)

```
+----------------------------------------------------------------------------------+
| HEADER: [Search customers] [Create segment] [Import contacts]                    |
+----------------------------------------------------------------------------------+
| NAV (left)                | MAIN: Customer List & Profile                         |
| ------------------------- | ---------------------------------------------------- |
| [Dashboard]               | LEFT: Customer List                                    |
| [CRM]                     |  - [Name] [Email] [Lifetime Value] [Orders]           |
| [Orders]                  | ---------------------------------------------------- |
| [Products]                | RIGHT: Customer Profile & Activity                     |
| [Promotions]              |  - Orders history, interactions, notes                 |
| [Settings]                | ---------------------------------------------------- |
|                          | SEGMENTS: dynamic/static segment builder               |
+----------------------------------------------------------------------------------+
| FOOTER: consent & GDPR tools                                                  |
+----------------------------------------------------------------------------------+
```

Shortcuts

:- [Full features doc](./features.md)
:- [Screens index](../index.md)

Navigation (clickable)

:- [Screens index](../index.md)
:- [Dashboard](../dashboard/index.md)
:- [Products](../products/index.md)
:- [Orders](../orders/index.md)
:- [Inventory](./inventory/index.md)
:- [Social](../social/index.md)
:- [Analytics](../analytics/index.md)
:- [Settings](./settings/index.md)
:- [Plugin Marketplace](../plugin_marketplace.md)
:- [Onboarding](../onboarding/index.md)
:- [Checkout](../checkout/index.md)
:- [Storefront Landing](../storefront_landing.md)
:- [Storefront Product](../storefront_product.md)

 
