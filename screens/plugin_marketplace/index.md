 # Plugin Marketplace — Browse, Install, Manage (Overview)

This folder contains the Plugin Marketplace screen spec and detailed feature documentation.

ASCII Layout (detailed)

```
+----------------------------------------------------------------------------------+
| HEADER: [Search plugins] [Categories] [Filter: free/paid] [Sort]                  |
+----------------------------------------------------------------------------------+
| NAV (left)                | MAIN: Plugin Cards Grid                              |
| ------------------------- | ---------------------------------------------------- |
| [Dashboard]               | PLUGIN CARD:                                         |
| [Promotions]              |  - logo, name, publisher, rating, price, install btn  |
| [Plugin Marketplace]      | ---------------------------------------------------- |
| [Settings]                | RIGHT RAIL: installed plugins, updates, settings      |
+----------------------------------------------------------------------------------+
| FOOTER: terms, developer portal links                                          |
+----------------------------------------------------------------------------------+
```

Shortcuts

:- [Full features doc](./features.md)
:- [Screens index](../index.md)

Navigation (clickable)

:- [Screens index](../index.md)
:- [Dashboard](../dashboard/index.md)
:- [Products](../products/index.md)
:- [Orders](../orders/index.md)
:- [Inventory](../inventory/index.md)
:- [CRM](../crm/index.md)
:- [Social](../social/index.md)
:- [Analytics](../analytics/index.md)
:- [Settings](../settings/index.md)
:- [Onboarding](../onboarding/index.md)
:- [Checkout](../checkout/index.md)
:- [Storefront Landing](../storefront_landing/index.md)
:- [Storefront Product](../storefront_product/index.md)

 
